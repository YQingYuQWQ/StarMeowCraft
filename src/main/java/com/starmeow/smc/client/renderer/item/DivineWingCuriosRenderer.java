...
@OnlyIn(Dist.CLIENT)
public class DivineWingCuriosRenderer implements ICurioRenderer {

    private final DivineWingModel model;

    public DivineWingCuriosRenderer() {
        this.model = new DivineWingModel(Minecraft.getInstance().getEntityModels().bakeLayer(DivineWingModel.LAYER_LOCATION));
    }
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(ItemStack stack,
        ...
        String wingTexture = DivineWing.wingTexture(stack);
        VertexConsumer vertexConsumer = renderTypeBuffer.getBuffer(
                RenderType.entityCutoutNoCull(new ResourceLocation(wingTexture))
        );
